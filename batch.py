...
outDir = '/home/scratch/kmpark/Z_20200710/'

dataDir = '/store/data/Run2016'
dataList = [ 'B_ver1', 'B_ver2', 'C', 'D', 'E', 'F', 'G', 'H' ]

mcDir = '/store/mc/RunIISummer16NanoAODv6-largeblock/'
mcList = [ 
           # HT binned DY jets 
...
for i in mcList:
    ...
    mcJobDic = {'jobName':i, 'outputFile':i+'.root', 'inputFile':mcDir+i, 'outputDir':outDir+i}
    runCode = '~/cpluostools/hadoopCondorSubmit.py {jobName} -e ../run0lep.py -o {outputFile} -d {inputFile} --outputdir {outputDir}'.format(**mcJobDic)
    print(runCode)
    os.system(runCode)
...
for i in dataList:
    ...
    dataJobDic = {'jobName':dataName, 'outputFile':dataName+'.root', 'inputFile':dataDir+i+'/SingleMuon/NANOAOD/'+dataVersion, 'outputDir':outDir+dataName}
    runCode = '~/cpluostools/hadoopCondorSubmit.py {jobName} -e ../run0lep.py -o {outputFile} -d {inputFile} --outputdir {outputDir}'.format(**dataJobDic)
    print(runCode)
    os.system(runCode)
